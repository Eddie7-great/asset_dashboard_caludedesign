...
- **Dividend tax has exactly one engine**: `allocateDividendTax(entries, threshold)` in `script.js`, on top of the rates in `getAccountDivTaxInfo(acc)` (일반 15.4% / ISA 9.9% + 200만원 공제 / 연금·IRP 과세이연). Every consumer routes through it — `syncDivHistory` (차트), `cbDivTaxAllocate` in `cobalt.js` (배당 관리 · a thin delegate that throws if the engine is missing), `cbDivMonthlyForYear` (캘린더), `finAccountDiagnostics` (계좌 배치 진단), `autoAddDividendCashFlow` (가계부 자동 등록).
  **The ISA exemption is per-owner-per-year, not per-holding.** Feed the engine *every* holding at once: it sums each owner's ISA dividends, applies the 200만원 exemption once, then splits the resulting tax across holdings pro rata. Applying it inside a per-item loop gives each holding its own exemption and silently zeroes the tax — this bug shipped twice, so never compute a rate inline.
  금융소득종합과세 (`CB_FIN_INCOME_THRESHOLD`, 2,000만원) is judged on **일반계좌 dividends only** — ISA is 분리과세 and 연금 is 과세이연.
