...
- ETF exploration uses a stable, selectable horizontal weight chart; never replace it asynchronously with 3D blocks. Korean constituents display company names; foreign tickers appear once beside the company name. Neighboring desktop cards stretch to equal height, and long lists retain compact pagination.
...
- 현금 안전판 목표 개월수(`cashTargetMonths`)는 **리스크 진단의 '현금 유동성 커버리지' 카드**에서 바꾼다(`finSaveCashTarget` → `cbRenderRisk`). 재무상태표 화면이 사라지면서 조작 지점이 없어졌던 값이다. 모바일은 읽기 전용이라 `finMobileNote` 를 렌더하고, `finSaveCashTarget` 이 없으면 컨트롤 자체를 그리지 않는다.
