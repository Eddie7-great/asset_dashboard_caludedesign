...
- Script order: `tax-rules.js` → `script.js` → `finance.js` → `navigation.js` → `backtest.js` → `simulator.js` → `etf-explorer.js` → `cobalt.js` → `allocation-3d.js`. The new modules define functions only until Cobalt renders. `workspace-ui.css` follows `style.css`.
...
- DCA 규칙 등록·수정은 종목 편집 모달이 맡는다(예전 인라인 편집기 `dca-editor.js` 는 호출부가 없어 지웠다). 적립식 화면에는 일정 확인과 일시정지 토글(`cbDcaToggle`, cobalt.js)만 있다 — 토글은 `dca` 플래그만 바꾸고, 원장·재무계획 두 원격 로드가 모두 ready 일 때만(`cbDcaCanSave`) 기존 CAS 저장 경로로 쓰며, 실패하면 되돌린 뒤 토스트로 알린다. No simulated fills.
...
- Scripts load in the order listed in Consolidated workspace above. `cobalt.js` wraps `switchView`, `changeOwner`, `saveAssetsToKV`, `loadAssetsFromKV`, `loadExtDataFromKV`, `fetchDivData`, `setTheme`, `liveRefresh`, `refreshPyData` to re-render the active page and stamp data-freshness. It must follow the finance/navigation/simulator/editor definitions it references in `CB_VIEWS`. The optional 3D enhancement loads after Cobalt.
...
- **실현손익 가져오기(세금 페이지 '거래내역 가져오기')는 파일에 적힌 소유주 × 연도만 교체한다.** `scripts/broker_realized_pl.py`(표준 라이브러리만: 삼성 xlsx·토스/메리츠 PDF 파서, 국내 이동평균·해외 원화 FIFO, 같은 날 거래내역서 환율 → `--fx-csv` 공개 일별 환율 순)가 `{format:'asset-dashboard/realized-pl', version:1, owner, years, rows}` 를 만들고, `cobalt.js` 의 순수 함수 `cbTaxImportPlan` 이 한 행이라도 틀리면 전부 거부한다. `ruleSetId` 는 파일 값이 아니라 `assetTaxRulesFor(연도)` 로 붙이고, 행은 `_normalizeMonthlyPLRows` 를 통과해야 한다. `cbTaxImportFile` 은 `_kvLoadState.ext` 가 ready 가 아니면 메모리를 바꾸기 전에 멈추고, `saveExtDataToKV` 실패 시 배열·localStorage 를 되돌린다. 모바일은 읽기 전용이라 버튼(`.cb-tax-import`)을 숨기고 `finMobileNote` 를 띄운다. 계산기는 매 거래일 끝에 원장 잔고와 대사해 불일치·원가 부족을 보고한다 — 조용히 넘기지 않는다. **가져오기 JSON·원본 거래내역은 개인 금융 정보라 커밋하지 않는다.** `test_tax_import.mjs`·`test_broker_realized_pl.py` 가 지킨다.
...
- **리밸런싱의 자산군 입력(`.fin-target-inputs`)은 4개씩 2줄로 고정한다** — `FIN_DEFAULT_TARGET`의 6개 자산군(가상화폐·미국·한국·일본·금·현금) + 허용 편차 + 목표 저장 버튼으로 정확히 8칸이다(`repeat(4,minmax(0,1fr))`, workspace-ui.css). `.fin-rebal-table`의 숫자 칼럼(현재·목표·편차·필요 조정액·월 DCA 보정안)은 우측 정렬한다(`>div>span:not(:first-child){text-align:right}`) — 첫 칸(자산군 라벨)만 아이콘+텍스트라 좌측 정렬로 둔다.
  `.fin-target-inputs` 규칙은 **workspace-ui.css 한 곳에만** 둔다 — 예전엔 두 파일 네 곳이 서로 다른 그리드로 덮어써 라벨이 '가상화'·'미국 주'로 잘렸다. 라벨은 '이름 / 입력·단위' 2단이다. 모바일(≤768px)에서 표는 옆으로 밀어 보되 자산군 칸을 고정하고 안내 문구를 띄운다. '계좌 배치 진단'의 평가액·비중도 숫자 열이라 오른쪽 정렬한다.
  **'현재 vs 목표' 막대(`finRebalBulletHtml`)** 는 `finTargetAnalysis` 의 current/target/threshold 를 그대로 그린다. 허용 범위 밖인 줄은 색이 아니라 '범위 밖' 글자로도 알린다.
- **패널 제목 모양은 하나(`.cb-panel-title`, `.cb-insight-title` 와 같은 값)다.** 인라인 `font-size:12px;letter-spacing:.08em;color:var(--lab)`(굵기 제각각)을 다시 쓰지 않는다 — 옆 카드끼리 제목 무게가 달라 보였다. 표 제목 줄(`.cb-family-table-toolbar`·`.cb-div-table-toolbar`·`.cb-dca-table-toolbar`, 내용 34px)과 옆 카드 제목 줄(`min-height:34px` 가운데 정렬)은 같은 높이여서 두 제목이 한 줄에 선다. 표 제목 줄 높이를 바꾸면 그 아래 sticky 칼럼 헤더의 `top:43px` 도 같이 바꾼다.
- **표 + 보조 위젯 2열(`.cb-family-detail-grid`·`.cb-dca-detail-grid`·`.cb-div-detail-grid`·`.fin-data-support-grid`·`.cb-gift-row`)은 `align-items:stretch`로 바닥선을 맞춘다.** 예외는 넓은 화면에서 긴 표를 따라 내려오는 sticky 카드(`.cb-family-mix-card`·`.cb-div-upcoming-card`)뿐이다 — sticky 는 행 높이로 늘어나면 동작하지 않으므로 `align-self:start` 를 유지한다.
- **리스크 카드 게이지(`cbRiskMeterHtml`)는 판정 기준값을 그대로 받는다.** 주요 카드는 `mk()` 의 `thWarn/thBad/invert`, 보조 카드는 `toneHigh/toneLow` 에 넘긴 같은 숫자를 `meter` 로 싣는다 — 한쪽만 바꾸면 색 판정과 게이지 구간이 어긋난다(`test_risk_visuals.mjs` 가 짝을 확인). 판정 보류 카드는 `meter:null` 로 그리지 않는다. 환노출 카드는 통화 구성 막대(`cbFxMixHtml`)로 그린다: 외화를 먼저, 원화를 끝에 쌓아 외화 길이가 곧 환노출이고, 색 `--fx-*` 는 `validate_palette` 를 라이트·다크 모두 통과한 범주 팔레트다(대비가 낮은 색이 있어 비중을 글자로도 적는다).
