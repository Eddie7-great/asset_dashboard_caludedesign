...
- **'비교 기준' 드롭다운은 `.etf-tabs` 와 같은 줄에 두고, `changes` 모드가 아닐 때 `visibility:hidden` 으로 자리를 유지한다.** 아래에 새 블록으로 생기면 '비중 변화'를 누를 때마다 그 아래가 통째로 밀린다. 숨긴 상태에서는 `disabled tabindex="-1"` 로 키보드 포커스도 받지 않게 한다.
