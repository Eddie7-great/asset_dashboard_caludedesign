...
- `etf-explorer.js` loads before `cobalt.js`; `etf-terrain.js` follows it. `etf2` is an asset-management tab, keeping eight main destinations and the related owner scope. The explorer never writes financial records.
...
- ETF terrain uses local Three.js with no idle animation or automatic rotation. It renders only on interaction/resize while visible, reuses its host, and disposes WebGL resources on exit. Mobile/WebGL failure keeps HTML bars, search and keyboard buttons. Long constituent lists use pagination; the compact layout must not clip financial amounts or remove access to secondary data.
...
- Script order: `tax-rules.js` → `script.js` → `finance.js` → `navigation.js` → `simulator.js` → `dca-editor.js` → `etf-explorer.js` → `cobalt.js` → `allocation-3d.js` → `etf-terrain.js`. The new modules define functions only until Cobalt renders. `workspace-ui.css` follows `style.css`.
