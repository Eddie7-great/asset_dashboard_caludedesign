...
- Script order: `tax-rules.js` → `script.js` → `finance.js` → `navigation.js` → `simulator.js` → `dca-editor.js` → `cobalt.js` → `allocation-3d.js`. The new modules define functions only until Cobalt renders. `workspace-ui.css` follows `style.css`.
