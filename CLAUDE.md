...
- `api/dashboard.py` (Python `BaseHTTPRequestHandler`) — multiplexed by `?type=` query param: `rates`, `gold`, `price`, `dividend`, `health`, `benchmark`, `fundamentals`, `resolve`, `etf_holdings`. Uses `yfinance` for global tickers and `pykrx` for KRX-only data (KR fundamentals, names, codes). `etf_holdings` returns ETF constituent weights (KR ETF → pykrx PDF, overseas ETF → yfinance `funds_data.top_holdings`) and feeds the risk page's ETF look-through concentration chart (`cbLookThrough` in `cobalt.js`, success responses day-cached in `localStorage` `etfHold_<YYYY-MM-DD>`). Always returns `{'success': bool, ...}`; on failure returns `'미조회'` instead of raising.
