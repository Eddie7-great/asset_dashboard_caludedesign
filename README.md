...
사이드바는 **일곱 개 메뉴**이며, 메뉴 안의 세부 화면은 제목 줄 오른쪽 링크로 전환합니다.

### 1. 홈
가족 투자자산 합계 + 일일손익·평가손익·연 배당·리스크 배지, 자산 배분 도넛(클릭 시 소속 종목 펼침), 섹터 집중도 바, 평가손익 기여도, 수익률·손실률 TOP 5, 그리고 검색 가능한 **보유 종목 표 + 선택 종목 요약**(배당 정보 포함).

### 2. 자산 관리
- **보유 목록** — 투자자산 CRUD + 적립식 매수 규칙 등록 (모든 화면의 원본)
- **ETF 탐색** — 보유 ETF의 구성종목, 기준일·완전성 점검, 소유주별 직접/간접 노출 분해, 날짜별 구성 변화
- **구성원 비교** — 구성원 카드 클릭 필터 + 보유 자산 표 + 소유주별 자산군 구성
- **비중 분석** — Plotly 선버스트로 비중 드릴다운

### 3. 투자 분석
- **성과** — 소유주별 포트폴리오 vs S&P 500 · KOSPI 정규화 차트와 기간별 초과수익·MDD 표. *현재 보유 비중을 기간 시작일부터 소급 적용한 백캐스트*이며 화면에 그렇게 표기됩니다. 같은 화면 아래의 **투자자산 추이** 카드는 소급 가정 없이 매일 기록된 실제 평가액을 그립니다(부동산·부채 제외, 수익률 아님)
- **리스크** — 점수 링 + 규칙 카드 8종(집중도·가상화폐·섹터·변동성·현금·환노출·분산·레버리지) + 보조 진단 9종(ETF 중복 노출률, **ETF 간 중복도**, 실효 종목 수, 환율 충격, 국가·섹터 집중도, 배당원 의존도, 현금 유동성 커버리지, 손실 회복 필요 수익률) + 종목 집중도 ETF 룩스루. 현금 안전판 **목표 개월수**를 여기서 바꿉니다

### 4. 배당
세전/세후 전환, 연간·월평균·YoC·배당성장률·집중도 카드, **입력된 일반계좌 배당 기준 금융소득 부분 근접도**, 월별 배당 캘린더, 종목별 배당 표(배당세 칼럼), 향후 90일 예상 일정.

### 5. 투자 계획
- **목표** — 재무 목표 진행률(자산군·순자산 연동), 남은 기간 기준 월 적립 필요액(수익률 0% 가정)
- **리밸런싱** — 목표 비중 대비 편차 → 필요 조정액 → 월 적립식 보정안, 계좌 배치 진단(실제 배당 세율 기준 절세 여력)
- **적립식 매수** — 규칙 상태, 다음 매수일·예상 처리 시점(시장 휴일·증권사별 주문 시각 반영), 월 환산 배분
- **투자 시뮬레이터** — 두 개 탭. **과거 성과 백테스트**(단일 종목, 수정주가 기준 적립식 시뮬레이션)와 **추가 투자 배분**(새로 넣을 돈을 목표 비중에 맞춰 나누는 메모리 전용 시나리오)

### 6. 현금 흐름
월별 수입·지출 가계부(일회성·자동이체), 수입/지출 구성과 순현금흐름 추이 차트, 고정비 관리. 여기서 고정비로 분류한 항목이 현금 안전판의 기준이 됩니다.

### 7. 세금·증여
- **실현손익·세금** — 월별 실현손익 기록·차트, 해외주식 250만원 기본공제 → 지방세 포함 22%, 계좌별(일반·연금저축·ISA) 구분, 연말 절세 여력과 손실 상계 후보
- **가족 증여** — 자녀 정기증여(법정 할인율을 월납으로 근사한 유기정기금 PV)와 부부 증여 계획, 최근 10년 실제 내역 대비 공제 잔여 검토
...
- `.github/workflows/` — 자동 배치. `품질 검사`(push·PR), `ETF 구성종목 수집`(미국 장 마감 뒤 평일 KST 07:30, 리포에 커밋), `순자산 스냅샷 기록`(매일 KST 16:40, KV에 직접 기록), `세금·증여 공식 근거 점검`(매일, 알림만)
