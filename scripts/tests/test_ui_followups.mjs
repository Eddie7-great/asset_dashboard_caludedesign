...
assert.match(scriptSource, /const _BUBBLE_SECTOR_COLORS = \{[\s\S]*'Technology':'#779BE0'[\s\S]*'Gold':'#CFB15F'/, '비중 차트에 섹터별 저채도 다색 팔레트 적용')
assert.match(scriptSource, /function _bubbleSectorColor\(sector\)[\s\S]*_bubbleBlend/, '비중 차트 팔레트를 테마별로 부드럽게 혼합')
