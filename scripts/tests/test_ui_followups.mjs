...
assert.match(cobaltSource, /\[rows\.slice\(0,3\),rows\.slice\(3,6\)\]/, '성과 비교 하단 기존 행 구조를 3개씩 두 묶음으로 분할')
...
assert.match(styleSource, /\.cb-dash-movers-grid\{display:grid!important;grid-template-columns:repeat\(2,minmax\(0,1fr\)\)!important/, '대시보드 수익률·손실률 TOP5를 가로 1×2로 강제')
...
assert.match(cobaltSource, /<span class="cb-tax-actions"[\s\S]*onclick="cbTaxEdit\([\s\S]*onclick="cbTaxDel\(/, '양도소득세 관리 열에 수정·삭제 버튼 제공')
...
assert.match(scriptSource, /function _bubbleSectorColor\(sector\)[\s\S]*cssVar/, '비중 차트 섹터 색상을 테마 토큰에서 계산')
