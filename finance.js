...
// 현금 안전판 목표 개월수를 바꾸는 유일한 자리는 투자 계획 > 목표다.
// (리스크 진단의 '현금 유동성 커버리지' 카드에 있었지만, 목표 화면이 이미 같은 값을
//  '현금 안전판 · 월 필수지출 기준'으로 보여주고 있어 바로 옆이 제자리다)
function finSaveCashTarget(){ const n=Math.max(1,Math.min(36,Number(document.getElementById('fin-cash-target')?.value)||6)); window._balanceSheet.cashTargetMonths=n; finSaveAndRender(typeof cbRenderPlan==='function'?cbRenderPlan:null); }
...
function finNwChartSvg(series,w,h){
  ...
  const step=cbNiceStep(span*1.2/6);   // 눈금 6단계 — 4단계는 한 칸이 너무 굵어 금액을 읽기 어려웠다
  ...
  window._finNwHover=series; window._finNwPts=pts;
  // 가이드선·마커는 SVG 안에 둔다 — 성과 차트처럼 HTML 오버레이로 하면 viewBox 의
  // padL/padR 을 퍼센트로 다시 계산해야 하는데, 여기서는 이미 정확한 좌표를 알고 있다.
  const guide=`<line id="fin-nw-guide" x1="0" x2="0" y1="0" y2="${h}" style="stroke:var(--acc);stroke-dasharray:3 3;display:none" stroke-width="1"></line>`
    +`<circle id="fin-nw-marker" r="4" fill="${stroke}" stroke="var(--inner-bg)" stroke-width="1.5" style="display:none"></circle>`;
...
// 마우스를 올린 지점이 그래프의 어디인지 세로 점선과 마커로 드러낸다.
// 터치는 기존 data-chart-hit="finNw:<i>" 문서 위임이 이 함수까지 그대로 이어진다.
function finNwGuide(idx){
  const g=document.getElementById('fin-nw-guide'), m=document.getElementById('fin-nw-marker');
  const pt=(window._finNwPts||[])[idx];
  if(!g||!m) return;
  if(!pt){ g.style.display='none'; m.style.display='none'; return; }
  g.setAttribute('x1',pt.x.toFixed(1)); g.setAttribute('x2',pt.x.toFixed(1)); g.style.display='';
  m.setAttribute('cx',pt.x.toFixed(1)); m.setAttribute('cy',pt.y.toFixed(1)); m.style.display='';
}
function finNwHide(){ const t=document.getElementById('cb-perf-tip'); if(t) t.style.display='none'; finNwGuide(-1); }
...
function finInvestTrendCard(){
  ...
  return `<div class="cb-panel fin-section" style="margin-top:12px">
    <div class="fin-section-head">
      ...
      <div class="fin-trend-tabs"><div class="owner-tabs" role="group" aria-label="소유주 선택">${ownerBtns}</div><span class="fin-trend-tabs-sep" aria-hidden="true"></span><div class="owner-tabs" role="group" aria-label="기간 선택">${tfBtns}</div></div>
    </div>
...
function finGoalPace(goal,current,now=new Date()){
  ...
  if(!Number.isFinite(gap))return '목표 금액 확인 필요';
  if(gap===0)return '목표 달성 · 다음 목표를 설정해 보세요';
  const days=(new Date(goal.targetDate+'T00:00:00')-now)/86400000;
  if(!Number.isFinite(days)||days<=0)return '남은 금액 '+cbDisp(gap)+' · 앞으로의 목표일을 설정해 주세요';
  const months=Math.max(1,Math.ceil(days/30.4375));
  return `남은 금액 ${cbDisp(gap)} · ${months}개월간 월 ${cbDisp(gap/months)} 필요 (수익률 0% 가정)`;
}
// 목표에 연결된 자산군 버킷. linkClass 가 CB_CLS 키(crypto/us/kr/jp/gold/cash)면
// 그 자산군으로, net/investment/manual 은 각각 순자산·투자자산·직접 입력 버킷으로
// 묶는다 — finGoalCurrent 가 이미 그 규칙으로 금액을 계산해 주므로 그대로 합산만
// 한다. 목표는 가구 공통이라(소유주 필드가 없다) 오너 필터를 받지 않는다.
const FIN_GOAL_BUCKET_META = {
  net:{label:'전체 순자산',color:'#94a3c8'},
  investment:{label:'가족 투자자산',color:'#7aa2ff'},
  manual:{label:'직접 입력',color:'#b48ead'},
};
function finGoalAllocation(){
  const buckets=new Map();
  (goalData||[]).forEach(g=>{
    const link=g.linkClass||'investment';
    const meta=FIN_GOAL_BUCKET_META[link]||CB_CLS[link];
    if(!meta)return;
    const cur=Math.max(0,Number(finGoalCurrent(g))||0);
    if(cur<=0)return;
    const b=buckets.get(link)||{key:link,label:meta.label,color:meta.color,value:0,goalCount:0};
    b.value+=cur;b.goalCount++;
    buckets.set(link,b);
  });
  return [...buckets.values()].sort((a,b)=>b.value-a.value);
}
// clickFn 을 넘기지 않는다 — onclick 이 붙으면 test_layout_guards.mjs 가
// role="button" tabindex="0" 을 요구한다. 이 도넛은 정보 표시용이라 상호작용이 없다.
function finGoalAllocationCard(){
  const buckets=finGoalAllocation();
  if(!buckets.length)return '';
  const total=buckets.reduce((s,b)=>s+b.value,0);
  const segs=buckets.map(b=>({pct:total>0?b.value/total*100:0,color:b.color}));
  return `<div class="cb-panel fin-goal-alloc-card"><div class="fin-section-head standalone"><span>목표 연결 자산군</span><small>목표에 연결된 자산 ${buckets.reduce((s,b)=>s+b.goalCount,0)}건 · 합계 ${cbDisp(total)}</small></div>
    <div class="fin-goal-alloc-body">
      <div class="fin-goal-alloc-donut">${cbDonutSvg(segs,116)}<div class="fin-goal-alloc-center"><b>${cbDisp(total)}</b><span>연결 자산 합계</span></div></div>
      <div class="fin-goal-alloc-legend">${buckets.map(b=>`<div class="fin-goal-alloc-item"><i style="background:${b.color}"></i><span>${cbEsc(b.label)}</span><b>${cbDisp(b.value)}</b><small>${total>0?(b.value/total*100).toFixed(1):'0.0'}%</small></div>`).join('')}</div>
    </div></div>`;
}
function finGoalContext(ownerF){
  const a=finTargetAnalysis(ownerF),cash=finCashSafety(ownerF);
  // 모바일은 재무 입력이 읽기 전용이라 저장 버튼 대신 현재 목표만 보여준다.
  const ro=(typeof isMobileLayout==='function'&&isMobileLayout());
  const ctl=ro
    ? `<div class="fin-cash-target"><span>목표 ${cash.targetMonths}개월</span>${typeof finMobileNote==='function'?finMobileNote('현금 안전판 목표'):''}</div>`
    : `<div class="fin-cash-target"><label for="fin-cash-target">목표 개월</label>`
      + `<input id="fin-cash-target" type="number" min="1" max="36" step="1" value="${cash.targetMonths}" aria-label="현금 안전판 목표 개월수">`
      + `<button onclick="finSaveCashTarget()">저장</button>`
      + `${cash.shortage>0?`<small>목표까지 ${cbDisp(cash.shortage)} 부족</small>`:''}</div>`;
  // ctl(목표 개월 입력·저장·부족액)을 <b> 앞으로 둔다 — 뒤에 두면 style.css 의
  // margin-top/border-top 이 행 높이를 키워, 옆 두 패널엔 없는 '3번째 층'이
  // 카드 맨 아래에 덩그러니 떠 보인다. 패널을 flex-column 으로 두어 순서 그대로
  // 위에서부터 쌓이게 한다.
...
function cbRenderPlan(){
  ...
  cbSetHead(rebalance?'목표 비중 편차 · 적립식 보정 · 계좌 배치 점검':'가구 공통 재무 목표와 진행률을 관리합니다',
  ...
  el.innerHTML=rebalance?`${finMobileNote('목표 비중')}    <div class="cb-panel fin-section"><div class="fin-section-head"><span>목표 비중과 리밸런싱 <span style="color:var(--dim);font-weight:500">· ${scope}</span></span><small>투자자산 ${cbDisp(ta.total)} · 월 DCA ${cbDisp(ta.monthlyDca)}${ownerF?' · 목표 비중값은 가구 공통':''}</small></div><div class="fin-rebal-comparison"><div class="fin-rebal-main"><div class="fin-target-inputs">${ta.result.map(x=>`<label><span><i style="background:${x.color}"></i>${cbEsc(x.label)}</span><input id="fin-target-${x.key}" type="number" min="0" max="100" step="1" value="${x.targetPct}"${ro}><em>%</em></label>`).join('')}<label class="threshold"><span>허용 편차</span><input id="fin-target-threshold" type="number" min="1" max="20" value="${ta.threshold}"${ro}><em>%p</em></label><button onclick="finSaveTarget()">목표 저장</button></div><div class="fin-rebal-table"><div class="head"><span>자산군</span><span>현재</span><span>목표</span><span>편차</span><span>필요 조정액</span><span>월 DCA 보정안</span></div>${ta.result.map(x=>`<div class="${Math.abs(x.drift)>=ta.threshold?'warn':''}"><span><i style="background:${x.color}"></i>${cbEsc(x.label)}</span><span>${x.currentPct.toFixed(1)}%</span><span>${x.targetPct.toFixed(1)}%</span><span class="${x.drift>=0?'up':'down'}">${x.drift>=0?'+':''}${x.drift.toFixed(1)}%p</span><span class="${x.diff>=0?'up':'down'}">${x.diff>=0?'매수 ':'축소 '}${cbDisp(Math.abs(x.diff))}</span><span>${x.dcaSuggestion>0?cbDisp(x.dcaSuggestion):'—'}</span></div>`).join('')}</div><small class="fin-disclaimer">조정액은 현재 평가액 기준의 단순 계산이며 매도·세금·거래비용을 반영하지 않습니다. DCA 보정안은 부족 자산군에 월 적립액을 비례 배분한 참고값입니다.</small></div>${finPortfolioReferences()}</div></div>
    ...
    <div class="fin-form-actions"><button class="primary" onclick="finGoalSubmit()">${ge?'수정 저장':'목표 추가'}</button>${ge?'<button onclick="finGoalCancel()">취소</button>':''}</div></div></div>
`;
}
...
function cbRenderDataStatus(){
  ...
  el.innerHTML=`<div class="fin-data-hero cb-panel"><div><small>데이터 신뢰 점검</small><strong>${ok}/${rows.length} 정상</strong><span>${cbEsc(heroNote)} 값이 비어 있거나 오래된 경우 먼저 이 화면에서 상태를 확인하세요.</span></div><button onclick="manualRefresh('all')">전체 데이터 새로고침</button></div><div class="fin-data-grid">${rows.map(x=>{
    ...
    const metaTip=`출처 · ${x.source||'내부 데이터'} · ${x.state==='pending'?'이번 접속에서 아직 확인하지 않음':`최근 확인 · ${finFreshAge(x.updatedAt)}${x.sameSession?'':' · 다른 접속 기록'}`}`;
    return `<div class="cb-panel fin-data-card ${st.cls}"><div class="fin-data-card-main"><div class="fin-data-card-head"><span class="fin-status-dot"></span><b>${cbEsc(x.label)}</b><em>${st.label}</em></div><strong class="cb-tip-block" data-overflow-tip="${cbEsc(x.detail||'상태 정보 없음')}" data-overflow-watch>${cbEsc(x.detail||'상태 정보 없음')}</strong><small class="fin-data-card-meta cb-tip-block" data-overflow-tip="${cbEsc(metaTip)}"><span data-overflow-watch>출처 · ${cbEsc(x.source||'내부 데이터')}</span><i aria-hidden="true">·</i><span data-overflow-watch>${when}</span></small></div><button onclick="manualRefresh('${x.key}')">다시 확인</button></div>`;
  }).join('')}</div><div class="fin-data-support-grid"><div class="cb-panel fin-section fin-price-help"><div class="fin-section-head"><span>시장 시세 확인 방법</span><div class="fin-price-actions"><button class="cb-btn" onclick="manualRefresh('prices')">시장 시세 재조회</button><button class="cb-btn" onclick="switchView('holdings')">보유 종목 확인</button></div></div><div class="fin-price-steps"><span><b>1. 재조회</b>미확인 종목이 사라지는지 확인</span><span><b>2. 종목 정보</b>티커·거래소·통화 대조</span><span><b>3. 증권사 비교</b>현재가 또는 전일 종가 확인</span></div><div class="fin-price-bottom"><div class="fin-price-issues">${(pfolioData||[]).filter(i=>i&&i._priceStale).map(i=>`<div><b>${cbEsc(i.name||i.tkr)}</b><span>${cbEsc(i.tkr)} · ${cbEsc(i.owner)} · ${cbEsc(i.cur||'통화 미확인')}</span><small>최신 조회 미확인 · 화면 가격은 이전 값일 수 있습니다.</small></div>`).join('')||'<p>현재 미확인 종목 없음 · 상태가 계속 ‘확인 필요’이면 재조회하세요.</p>'}</div><p class="fin-price-note">한국은 6자리 코드, 일본은 거래소 코드가 필요합니다. ‘정상’은 조회 성공이며 실시간 체결가를 보장하지 않습니다.</p></div></div><div class="cb-panel fin-section"><div class="fin-section-head"><span>상태 해석</span><small>투자 판단 전에 데이터 시점을 확인하세요.</small></div><div class="fin-guidance"><p><b>정상</b>은 해당 데이터의 마지막 요청이 성공했다는 뜻입니다.</p><p><b>확인 필요</b>는 요청이 실패했거나 시세가 누락된 자산이 남아 있다는 뜻입니다.</p><p><b>확인 전</b>은 아직 조회를 실행하지 않은 상태로, 오류가 아닙니다.</p><p>최근 확인 시각은 기기별로 공유되므로 <b>다른 접속 기록</b> 표시가 붙은 항목은 다시 확인해 주세요.</p><p>외부 데이터는 거래소·제공사 사정에 따라 지연될 수 있으며, 주문 전 실제 증권사 시세를 확인해야 합니다.</p></div></div></div>`;
}
