...
function cbRenderDataStatus(){
  ...
  }).join('')}</div><div class="cb-panel fin-section fin-price-help"><div class="fin-section-head"><span>시장 시세 확인 방법</span><button class="cb-btn" onclick="manualRefresh('prices')">시장 시세 재조회</button></div><p>① 재조회 후 아래 미확인 종목이 사라지는지 확인하세요. ② 보유 목록의 티커·거래소·통화를 확인하세요. 한국 종목은 6자리 코드, 일본 종목은 원래 거래소 코드가 필요합니다. ③ 거래 중인 증권사에서 같은 종목의 현재가 또는 전일 종가를 대조하세요. 장중 가격과 전일 종가는 서로 다를 수 있습니다.</p><div class="fin-price-issues">${(pfolioData||[]).filter(i=>i&&i._priceStale).map(i=>`<div><b>${cbEsc(i.name||i.tkr)}</b><span>${cbEsc(i.tkr)} · ${cbEsc(i.owner)} · ${cbEsc(i.cur||'통화 미확인')}</span><small>최신 조회 미확인 · 화면 가격은 이전 값일 수 있습니다.</small></div>`).join('')||'<p>현재 미확인 표시가 붙은 종목은 없습니다. 상태가 계속 ‘확인 필요’이면 위 재조회로 이번 접속의 결과를 확인하세요.</p>'}</div><p>재조회에도 실패하면 종목 코드 변경·상장폐지 여부와 제공사 일시 장애를 확인하세요. ‘정상’은 조회 성공을 뜻하며 실시간 체결가를 보장하지 않습니다.</p><button class="cb-btn" onclick="switchView('holdings')">보유 목록에서 종목 확인</button></div><div class="cb-panel fin-section"><div class="fin-section-head"><span>상태 해석</span><small>투자 판단 전에 데이터 시점을 확인하세요.</small></div><div class="fin-guidance"><p><b>정상</b>은 해당 데이터의 마지막 요청이 성공했다는 뜻입니다.</p><p><b>확인 필요</b>는 요청이 실패했거나 시세가 누락된 자산이 남아 있다는 뜻입니다.</p><p><b>확인 전</b>은 아직 조회를 실행하지 않은 상태로, 오류가 아닙니다.</p><p>최근 확인 시각은 기기별로 공유되므로 <b>다른 접속 기록</b> 표시가 붙은 항목은 다시 확인해 주세요.</p><p>외부 데이터는 거래소·제공사 사정에 따라 지연될 수 있으며, 주문 전 실제 증권사 시세를 확인해야 합니다.</p></div></div>`;
}
