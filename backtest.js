...
let _bt={query:'',selected:null,range:'5y',initial:10000,monthly:0,fee:0,result:null},_btEpoch=0,_btController=null,_btSearchTimer=null;
...
function btHtml(){
  return `<section class="cb-panel bt-panel"><div class="fin-section-head"><span>종목 백테스팅</span><small>과거 수정주가 · 일시 투자 + 월 적립</small></div><div class="bt-controls"><label class="bt-query-field">종목명 / 티커<input id="bt-query" value="${cbEsc(_bt.query)}" placeholder="예: 삼성전자, NVIDIA, 7203.T" autocomplete="off" oninput="btQuery(this.value)"></label><label>조회 기간<select id="bt-range" onchange="btSetting('range',this.value)">${['1y','2y','5y','10y'].map(v=>`<option value="${v}"${v===_bt.range?' selected':''}>${v.replace('y','년')}</option>`).join('')}</select></label><label>초기 투자금<input id="bt-initial" type="number" data-no-comma="1" min="1" max="1000000000000" value="${_bt.initial}" oninput="btSetting('initial',this.value)"></label><label>월 적립금<input id="bt-monthly" type="number" data-no-comma="1" min="0" max="10000000000" value="${_bt.monthly}" oninput="btSetting('monthly',this.value)"></label><label>매수 수수료 %<input id="bt-fee" type="number" data-no-comma="1" min="0" max="5" step="0.01" value="${_bt.fee}" oninput="btSetting('fee',this.value)"></label><button id="bt-run" class="cb-btn" onclick="btRun()"${_bt.selected?'':' disabled'}>백테스트 실행</button></div><div class="bt-search-feedback"><span id="bt-selection">${_bt.selected?cbEsc(_bt.selected.name+' · '+_bt.selected.symbol+' · '+_bt.selected.currency):'종목명이나 티커를 입력하면 자동 검색합니다.'}</span><div id="bt-candidates" role="status" aria-live="polite"></div></div><p id="bt-status" role="status"></p><div id="bt-output">${_bt.result?btResultHtml(_bt.result):''}</div><p class="sim-assumptions">첫 거래일 종가에 초기 투자하고, 다음 달부터 각 월의 첫 조회 거래일 종가에 적립합니다. 소수점 매수·배당 재투자를 수정주가로 근사합니다. 환율 변동·세금·슬리피지·매도 비용은 제외하며 매수 수수료만 반영합니다. 조회 기간이 상장 이력보다 길면 실제 제공 기간으로 계산합니다. 과거 성과는 미래 수익을 보장하지 않습니다. 원장과 실제 주문은 변경하지 않습니다.</p></section>`;
}
function btInvalidate(){_btEpoch++;_btController?.abort();clearTimeout(_btSearchTimer);_btSearchTimer=null;_bt.result=null;const out=document.getElementById('bt-output');if(out)out.innerHTML='';const status=document.getElementById('bt-status');if(status)status.textContent='설정한 조건으로 실행해 주세요.';}
function btSetting(key,value){btInvalidate();_bt[key]=key==='range'?value:String(value).trim()===''?NaN:Number(value);}
function btQuery(value){btInvalidate();_bt.query=value;_bt.selected=null;_bt.candidate=null;const selection=document.getElementById('bt-selection'),box=document.getElementById('bt-candidates'),run=document.getElementById('bt-run');if(selection)selection.textContent=value.trim()?'입력을 멈추면 자동 검색합니다.':'종목명이나 티커를 입력하면 자동 검색합니다.';if(box)box.replaceChildren();if(run)run.disabled=true;if(value.trim()&&value.length<=80)_btSearchTimer=setTimeout(btSearch,350);}
...
function btResultHtml(r){
  const max=Math.max(...r.points.map(p=>Math.max(p.value,p.paid)),1),w=1000,h=280,left=94,right=18,top=18,bottom=18,plotW=w-left-right,plotH=h-top-bottom;
  const y=value=>top+(1-value/max)*plotH;
  const line=key=>r.points.map((p,i)=>(left+i/(r.points.length-1)*plotW).toFixed(1)+','+y(p[key]).toFixed(1)).join(' ');
  const axis=value=>Number(value).toLocaleString('ko-KR',{notation:'compact',maximumFractionDigits:1});
  const ticks=Array.from({length:5},(_,i)=>{const value=max*(4-i)/4,py=y(value);return `<g class="bt-y-tick"><line x1="${left}" x2="${w-right}" y1="${py}" y2="${py}"/><text x="${left-10}" y="${py+4}" text-anchor="end">${axis(value)}</text></g>`;}).join('');
  return `<div class="bt-kpis">${[['납입 원금',btMoney(r.paid)],['최종 평가액',btMoney(r.value)],['납입원금 대비 손익률',(r.roi*100).toFixed(2)+'%'],['종목 CAGR',(r.cagr*100).toFixed(2)+'%'],['수정주가 최대 낙폭',(r.mdd*100).toFixed(2)+'%']].map(([k,v])=>`<div><small>${k}</small><b>${v}</b></div>`).join('')}</div><p>평가손익 ${btMoney(r.profit)} · 누적 매수 수수료 ${btMoney(r.fees)} · CAGR/낙폭은 적립금과 무관한 수정주가 기준입니다.</p><svg class="bt-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="시간에 따른 평가금액과 누적 납입원금. 세로축은 ${cbEsc(_bt.selected?.currency||'종목 통화')} 평가금액">${ticks}<text class="bt-y-title" transform="translate(18 ${h/2}) rotate(-90)" text-anchor="middle">평가금액 · ${cbEsc(_bt.selected?.currency||'종목 통화')}</text><polyline class="bt-line bt-line-paid" points="${line('paid')}" fill="none" stroke="var(--lab)" stroke-width="2" stroke-dasharray="6 4"/><polyline class="bt-line bt-line-value" points="${line('value')}" fill="none" stroke="var(--acc)" stroke-width="3"/></svg><div class="bt-chart-labels"><span>${r.points[0].date}</span><span>실선 평가액 · 점선 납입원금</span><span>${r.points[r.points.length-1].date}</span></div><label class="bt-inspect">거래일 살펴보기<input type="range" min="0" max="${r.points.length-1}" value="${r.points.length-1}" oninput="btInspect(this.value)"></label><p id="bt-inspect">${btPointText(r.points[r.points.length-1])}</p>`;
}
