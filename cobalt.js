...
function cbRenderTax(){
  ...
  el.innerHTML = `
    ...
    <!-- 하단: 실현손익 기록 + 내역 (메모 포함) -->
    <div class="cb-panel" style="margin-top:12px;padding:14px 16px">
      <div style="font-size:10.5px;letter-spacing:.08em;color:var(--lab);margin-bottom:10px">실현손익 기록 · ${year}년 내역 <span style="color:var(--dim)">· 해외주식은 일반 계좌만 선택 가능합니다</span></div>
...
function cbRenderDca(){
  ...
  el.innerHTML = `
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <div class="cb-panel" style="flex:1;min-width:150px;padding:12px 14px"><div style="font-size:11px;color:var(--lab)">활성 규칙${ownerF?' · '+cbEsc(ownerF):''}</div><div style="font-family:'Manrope','Noto Sans KR',sans-serif;font-size:22px;font-weight:800;margin-top:2px">${active.length}<span style="font-size:13px;color:var(--lab)"> / ${items.length}</span></div></div>
