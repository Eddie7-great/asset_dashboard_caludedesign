...
`script.js`가 직접 렌더하는 화면(자산 내역·현금 흐름·비중 차트)은 `<div id="view-X">` 안에 마크업이 이미 들어있는 구조로, `CB_LEGACY_SUB`에 헤더 소제목만 등록합니다. 메뉴에서 도달할 수 없던 레거시 뷰 7개(`dashboard`·`portfolio`·`dividend`·`gift`·`family`·`analysis`·`target_rebal`)는 제거됐고, 현재 DOM에 있는 15개 뷰는 모두 사이드바나 푸터에서 열립니다.
